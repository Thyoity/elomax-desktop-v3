<template>
  <div class="services--container my-services">
    ...
    <div v-if="!isLoadingServices" class="page-content">
      ...
      <OverlayScrollbar v-if="inProgressEloBoosts && inProgressEloBoosts.length> 0"
        style="height: calc(100% - 100px)"
      >
        <div class="service-list">
          <div
            v-for="eloBoost in inProgressEloBoosts"
            :key="eloBoost.id"
          ...
          >
            ...
            >
              ...
              <h5 class="client">
                Cliente: {{ eloBoost.client.username }}
                <div
                  class="user-status"
                  :class="{ online: eloBoost.client.isOnline }"
                ></div>
              </h5>
              <button @click="openService(eloBoost)" class="open-service">
                Acompanhar
              </button>
            </div>
              ...
              <div class="start-point">
                ...
                <span style="white-space: nowrap">
                  ...
                  <small v-if="eloBoost.details.initial_tier !== 'unranked'"
                    >({{ eloBoost.details.initial_lp }} LP's)</small
                  >
                </span>
              </div>
            ...
            <div class="service-details">
              <ul>
                <li class="detail deadline">
                  ...
                  ></strong>
                  <span v-if="eloBoost.details.date_deadline"
                    >Até {{ formatDate(eloBoost.details.date_deadline) }}</span
                  >
                  <span v-else>---</span>
                </li>
                <li class="detail earning">
                  <strong><ion-icon name="wallet-outline"></ion-icon></strong>
                  <span v-if="eloBoost.boosterAmount"
                    >R$ {{ eloBoost.boosterAmount.replace(".", ",") }}</span
                  >
                  <span v-else>---</span>
                </li>
              </ul>
...
export default {
  ...
  computed: {
    ...
    ...mapState(useServicesStore, ["inProgressEloBoosts"]),
  },
  methods: {
